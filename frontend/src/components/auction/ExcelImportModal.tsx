...
export const ExcelImportModal: React.FC<ExcelImportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-4xl rounded-3xl bg-[#0c1017] border border-white/15 p-6 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Fuzzy column mapping, strict Zod validation, and dry-run error inspector
              </p>
            </div>
          </div>
          <button
        ...
        {stage === 'UPLOAD' && (
          <div className="space-y-4">
            <div
              ...
              className="border-2 border-dashed border-white/15 hover:border-amber-400/50 rounded-3xl p-12 text-center cursor-pointer transition-all hover:bg-white/[0.02] group"
            >
              <div className="w-16 h-16 mx-auto rounded-2xl bg-amber-500/10 group-hover:bg-amber-500/20 border border-amber-500/30 flex items-center justify-center mb-4 transition-colors">
              ...
              <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/10 text-xs font-semibold text-slate-200 border border-white/10 group-hover:bg-white/15">
            ...
            <div className="p-4 rounded-2xl bg-black/40 border border-white/5 flex items-center justify-between text-xs text-slate-400">
              <div className="flex items-center gap-2">
                ...
                <span>Default Sample: <strong className="text-slate-200">{fileName}</strong></span>
              </div>
              <span>{fileSize}</span>
            </div>
          </div>
        ...
        {stage === 'MAPPING' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-emerald-400 font-mono">10/10 Headers Inferred</span>
            </div>

            <div className="rounded-2xl border border-white/10 overflow-hidden">
              <table className="w-full text-xs text-left">
                <thead className="bg-black/50 text-slate-400 uppercase tracking-wider text-[10px]">
                  <tr>
                    ...
                    <tr key={idx} className="hover:bg-white/[0.02]">
                      ...
                      <td className="p-3">
                        <select
                          ...
                          className="bg-black/60 border border-white/10 rounded-lg px-2.5 py-1 text-slate-200 focus:outline-none focus:border-amber-400"
                        >
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-amber-400 text-black font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-amber-500/20"
              >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-2xl bg-black/40 border border-white/10 text-xs">
                <span className="text-slate-400 block mb-1">Total Catalog Lots</span>
              ...
              <div className="p-3 rounded-2xl bg-red-950/30 border border-red-500/30 text-xs">
            ...
            {validationReport.errors.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="rounded-2xl border border-red-500/30 bg-red-950/10 p-3 space-y-2 max-h-40 overflow-y-auto">
                  {validationReport.errors.map((err, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 rounded-xl bg-black/40 border border-red-500/20 text-xs flex items-start justify-between"
                    >
                      <div>
                      ...
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-red-500/20 text-red-400">
                        Field: {err.field}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-black font-extrabold text-xs flex items-center gap-2 shadow-lg shadow-emerald-600/30"
              >
                {isProcessing ? (
                  <>
        ...
        {stage === 'COMMITTED' && (
          <div className="space-y-5 text-center py-6">
            ...
            <div>
              ...
              <div className="mt-3 inline-block font-mono text-xs text-amber-400 bg-black/40 px-3 py-1 rounded-lg border border-white/10">
                Batch ID: {committedBatchId}
              </div>
            </div>

            {/* Rollback Safety Mechanism */}
            <div className="p-4 rounded-2xl bg-red-950/20 border border-red-500/30 text-left max-w-xl mx-auto flex items-center justify-between">
              <div>
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-red-500/20 hover:bg-red-500/30 border border-red-500/40 text-xs font-bold text-red-300 flex items-center gap-1.5 transition-colors"
              >
            ...
            <div className="pt-2">
              <button
                onClick={onClose}
                className="px-6 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-xs font-semibold text-white border border-white/15"
              >
